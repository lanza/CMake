...
public:
  ...
  struct CanonicalFileLocation {
    ...
    CanonicalFileLocation(const std::string &path = "", int line = 0)
        : Path(path), OneBasedLine(line) {}
  };
  ...
  struct CaseInsensitiveObjectName {
    ...
    bool operator<(const CaseInsensitiveObjectName &right) const {
      return cmsysString_strcasecmp(Name.c_str(), right.Name.c_str()) < 0;
    }
  ...
  struct BreakpointObject {
    ...
    BreakpointObject(UniqueBreakpointID id,
                     const CanonicalFileLocation &location)
        : Location(location) {
      AssignedID = id;
    }
    BreakpointObject(UniqueBreakpointID id,
                     const CaseInsensitiveObjectName &name)
        : FunctionName(name) {
      AssignedID = id;
    }
    BreakpointObject(
        UniqueBreakpointID id,
        std::unique_ptr<DomainSpecificBreakpointExtension> &&extension)
...
public:
  BreakpointObject *TryGetBreakpointAtLocation(const std::string &file,
                                               int oneBasedLine);
  BreakpointObject *TryGetBreakpointForFunction(const std::string &function);

  UniqueBreakpointID CreateBreakpoint(const std::string &file,
                                      int oneBasedLine);
  UniqueBreakpointID CreateBreakpoint(const std::string &function);
  UniqueBreakpointID CreateDomainSpecificBreakpoint(
      std::unique_ptr<DomainSpecificBreakpointExtension> &&extension);
  ...
  UniqueBreakpointID
  TryLocateEnabledDomainSpecificBreakpoint(_Predicate predicate) {
...
private:
  ...
  std::map<UniqueBreakpointID, std::unique_ptr<BreakpointObject>>
      m_BreakpointsByID;
  std::map<CanonicalFileLocation, std::set<UniqueBreakpointID>>
      m_BreakpointsByLocation;
  std::vector<std::pair<CanonicalFileLocation, std::set<UniqueBreakpointID>*>>
      m_fileline_breakpoints;
  std::map<CaseInsensitiveObjectName, std::set<UniqueBreakpointID>>
      m_BreakpointsByFunctionName;

  std::map<std::string, std::string> m_CanonicalPathMap;

  CanonicalFileLocation MakeCanonicalLocation(const std::string &file,
                                              int oneBasedLine);
};
} // namespace sp
