...
public:
  TargetExpression(cmTarget* pTarget)
    : m_pTarget(pTarget)
  {
    Type = "(CMake target)";
    ...
    override
  {
    ...
    for (const auto& kv : m_pTarget->GetProperties())
      result.push_back(std::make_unique<SimpleExpression>(
        kv.first, "(property entry)", kv.second.GetValue()));
    return result;
  }
};
#pragma endregion
...
class HLDPServer::DomainSpecificBreakpoint
  : public BasicBreakpointManager::DomainSpecificBreakpointExtension
{
public:
  ...
  DomainSpecificBreakpoint(const std::string& stringArg, int intArg)
    : Type((CMakeDomainSpecificBreakpointType)intArg)
    , StringArg(stringArg)
  {
  }
};
...
std::unique_ptr<HLDPServer::RAIIScope> HLDPServer::OnExecutingInitialPass(
  ...
  const cmListFileFunction& function, bool& skipThisInstruction)
{
  ...
  if (!m_BreakInPending) {
    if (m_pSocket->HasIncomingData()) {
      ...
      switch (requestType) {
        ...
        default:
          if (requestType >
                HLDPPacketType::BeforeFirstBreakpointRelatedCommand &&
              requestType <
                HLDPPacketType::AfterLastBreakpointRelatedCommand) {
...
void HLDPServer::OnMessageProduced(unsigned rawType,
                                   const std::string& message)
{
  ...
  auto id = m_BreakpointManager.TryLocateEnabledDomainSpecificBreakpoint(
    [&](BasicBreakpointManager::DomainSpecificBreakpointExtension* pBp) {
    ...
    [&](BasicBreakpointManager::DomainSpecificBreakpointExtension* pBp) {
      switch (static_cast<DomainSpecificBreakpoint*>(pBp)->Type) {
        ...
        case CMakeDomainSpecificBreakpointType::VariableUpdated:
          if (isRead ==
                (static_cast<DomainSpecificBreakpoint*>(pBp)->Type ==
      ...
      if (!reader.ReadInt32(&intArg1) || !reader.ReadString(&stringArg) ||
        ...
        if (!id)
          ...
          if (intArg1 ==
              ...
              intArg1 ==
                (int)CMakeDomainSpecificBreakpointType::VariableUpdated)
...
void HLDPServer::ReportStopAndServeDebugRequests(
  TargetStopReason stopReason, unsigned intParam,
  const std::string& stringParam, bool* pSkipThisInstruction)
{
  ...
  for (;;) {
    ...
    switch (requestType) {
      ...
      case HLDPPacketType::csStepOver:
        if (m_CallStack.size() >= 2)
          m_EndOfStepScopeID =
            m_CallStack[m_CallStack.size() - 2]->GetUniqueID();
        else
          m_EndOfStepScopeID = kRootScope;
        SendReply(HLDPPacketType::scTargetRunning, builder);
        return;
      ...
      case HLDPPacketType::csCreateExpression: {
        ...
        if (ID < 0 || ID >= m_CallStack.size())
          ...
          if (pExpression) {
            ...
            m_ExpressionCache[pExpression->AssignedID] =
...
HLDPServer::RAIIScope::~RAIIScope()
{
  ...
  if (m_UniqueID == m_pServer->m_EndOfStepScopeID)
    m_pServer->m_BreakInPending =
      true; // We are stepping out of a function scope where we were supposed
            // to stop.

  m_pServer->m_CallStack.pop_back();
}

std::unique_ptr<HLDPServer::ExpressionBase> HLDPServer::CreateExpression(
  const std::string& text, const RAIIScope& scope)
{
