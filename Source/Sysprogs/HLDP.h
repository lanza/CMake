...
namespace Sysprogs {
/*
                This file defines structures used in the Sysprogs High-Level
   Debug Protocol. The protocol is optimized for low latency and is designed to
   run on resource-limited embedded devices, hence all exchange is performed
...
/*
                ...
                Common terms:
                * Subexpression delimiter - an operator (e.g. '.') that should
   never be a part of a valid expression for this target. The client will
   internally use it to make fully qualified paths of subexpressions and will
   automatically break it down into separate requests so the server doesn't
   need to do any advanced parsing.

                Data types:
                * Array := [length:int32] [element #0] [element #1] ... [last
   element]
   ...
   CMake domain-specific breakpoint payload:
                type:int32 (CMakeDomainSpecificBreakpointType),
   StringArg:string, reserved:int32
*/
} // namespace Sysprogs
